...
def _process_roi_data(data,data_type,tr_secs,x_coord_name,y_coord_name, seed_roi=None):
    """ 
    ...
    data             = data.copy()
    if ('roi' in y_coord_name) or ('ROI' in y_coord_name):
        ...
        data             = data.sel({y_coord_name:new_rois_order})
    ...
    elif ('edge' in y_coord_name) or ('EDGE' in y_coord_name):
        aux_idx_tuple = [tuple(e.split('-')[0].split('_',2)+e.split('-')[1].split('_',2)) for e in data.coords[y_coord_name].values]
        aux_idx_midx  = pd.MultiIndex.from_tuples(aux_idx_tuple,names=['ROIx_Hemisphere','ROIx_Network','ROIx_ROI','ROIy_Hemisphere','ROIy_Network','ROIy_ROI'])
        # This line ensures that the seed ROIs are always the x component of the index
        aux_idx_tuple_seed_first = [tuple(e.split('-')[0].split('_',2)+e.split('-')[1].split('_',2)) if e.split('-')[0] == seed_roi else tuple(e.split('-')[1].split('_',2)+e.split('-')[0].split('_',2))  for e in data.coords[y_coord_name].values]
        aux_idx_midx_seed_first  = pd.MultiIndex.from_tuples(aux_idx_tuple_seed_first,names=['ROIx_Hemisphere','ROIx_Network','ROIx_ROI','ROIy_Hemisphere','ROIy_Network','ROIy_ROI'])
        
        aux_idx_midx_seed_first_sorted,sorting = aux_idx_midx_seed_first.sortlevel('ROIy_Network')
        new_edge_order   = data.coords[y_coord_name][sorting]
        data             = data.sel({y_coord_name:new_edge_order})
        # Once data is sorted, we can now go ahead and extract network names, etc...
        nw_names         = list(aux_idx_midx_seed_first_sorted.get_level_values('ROIy_Network').unique())
    else:
        print('++ ERROR[_process_roi_data]: Cannot understand the data. ROI or EDGE is not the y dimension.')
        return None

    num_nws          = len(nw_names)

    # Create a dictionary linking an ID [0..max] to each roi/edge name post-sorting
    y_name_2_y_idx = {roi:i for i,roi in enumerate(data.coords[y_coord_name].values)}
    ...
    if ('edge' in y_coord_name) or ('EDGE' in y_coord_name):
        aux_idx_tuple_seed_first = [tuple(e.split('-')[0].split('_',2)+e.split('-')[1].split('_',2)) if e.split('-')[0] == seed_roi else tuple(e.split('-')[1].split('_',2)+e.split('-')[0].split('_',2))  for e in data_df.index.get_level_values(y_coord_name)]
        aux_idx_midx_seed_first  = pd.MultiIndex.from_tuples(aux_idx_tuple_seed_first,names=['ROIx_Hemisphere','ROIx_Network','ROIx_ROI','ROIy_Hemisphere','ROIy_Network','ROIy_ROI'])
        
        data_df       = data_df.reset_index()
        data_df.index = aux_idx_midx_seed_first
        
    data_df[y_coord_name] = [y_name_2_y_idx[i] for i in list(data_df[y_coord_name].values)]
    return data, data_df, nw_names, schedule, tap_label, x_coord_name,y_coord_name, y_name_2_y_idx
...
def create_roits_figure(data,data_type,tr_secs,
    ...
    roits['Hemisphere'] = [roi_idx_2_hemi[i] for i in roits[y_coord_name].values]
    roits['Network']    = [roi_idx_2_nw[i] for i in roits[y_coord_name].values]
    roits['ROI_Name']   = [roi_idx_2_name[i] for i in roits[y_coord_name].values]
...
def create_ets_figure(data,data_type,tr_secs,
                      ...
                      time_segment='All', hrf_thr=0.2):
    """
    Creates a figure with four plots: 
    (1) A carpet plot of the Edge timeseries sorted and organized by network.
    (2) Timeseries or RSS across all Edges included in the carpet plot & including only edges of each network
    (3) Seed ROI Timeseries
    (4) Trace of RSS across a given scan segment for all ROIs in the plot.
    ...
    x_coord_name: name of coordinate to be used as X-axis.
    y_coord_name: name of coordinate to be used as Y-axis.
    seed_roi: seed ROI name (e.g., HEMI_NW_ROI-NAME).
    echo: trace to show (e.g., E01, E02, E03)
    roi_ts: dataframe with seed ROI timeseries. Two columns expected (Time [sec] & BOLD)
    width: total figure width
    height: total figure height
    cmap: colormap for the carpet plot
    vmin,vmax: min and max values for the carplet plot colorbar
    ...
    # Extract basic information
    # =========================
    data, data_df, nw_names, schedule, tap_label,x_coord_name,y_coord_name,y_name_2_y_idx  = _process_roi_data(data,data_type,tr_secs,x_coord_name,y_coord_name, seed_roi=seed_roi)
    Nrois = data.coords[y_coord_name].shape[0]
    Nacqs = data.coords[x_coord_name].shape[0]
    ...
    # 1. Yticks and Yticklabels
    num_nws       = len(nw_names)
    aux_idx_tuple = [tuple(e.split('-')[0].split('_',2)+e.split('-')[1].split('_',2)) if e.split('-')[0] == seed_roi else tuple(e.split('-')[1].split('_',2)+e.split('-')[0].split('_',2))  for e in data.coords[y_coord_name].values]
    
    data_edge_midx = pd.MultiIndex.from_tuples(aux_idx_tuple,names=['ROIx_Hemisphere','ROIx_Network','ROIx_ROI','ROIy_Hemisphere','ROIy_Network','ROIy_ROI'])

    edges_per_nw  = data_edge_midx.get_level_values('ROIy_Network').value_counts().to_dict()
    edges_per_nw  = np.array([edges_per_nw[nw] for nw in nw_names])
    net_edges     = [0]+ list(edges_per_nw.cumsum())                                    # Position of horizontal dashed lines for nw delineation
    yticks        = [int(i) for i in net_edges[0:-1] + np.diff(net_edges)/2]           # Position of nw names
    yticks_info   = list(tuple(zip(yticks, nw_names)))
    # 2. Colored segments
    nw_col_segs = hv.Segments((tuple(-0.5*np.ones(num_nws)),
    ...
    # -----------
    cplot = data_df.reset_index().hvplot.heatmap(x=x_coord_name,y=y_coord_name,C=data_type, width=int(0.8*width), height=int(0.8 * height),
                                         cmap=cmap, hover_cols=['ROIx_Hemisphere','ROIx_Network','ROIx_ROI','ROIy_Hemisphere','ROIy_Network','ROIy_ROI'],
                                         ylabel='%s edges from %s' % (str(Nrois),str(seed_roi)), yticks=yticks_info, 
                                         clim=(vmin,vmax)).opts(xrotation=90,colorbar_opts={'title':data_type})
    cplot = cplot * nw_col_segs
    cplot = cplot * task_annot
    cplot.opts(xlim=(-0.5,data_df[x_coord_name].max()),ylim=(0,Nrois))
    
    # Root Sum of Squares Time series across the brain & per network
    # --------------------------------------------------------------
    # First, we compute it across all ROI (Full Brain)
    rssts = data_df.groupby(x_coord_name).agg(root_sum_of_squares)[data_type]
    rssts.name = 'RSS TS'
    rssts = pd.DataFrame(rssts).reset_index()
    time_idx = rssts['Time [sec]']
    # Extract min and max if not provided
    if rssts_min is None:
        rssts_min = rssts['RSS TS'].min()
    if rssts_max is None:
    ...
    # Root Sum of Squares across time for each ROI separately
    # -------------------------------------------------------
    # Get onset and offsets per task
    if data_type == 'BOLD':
        task_onsets, task_offsets = get_hrf_event_onsets_offsets(schedule,tap_label,tr_secs,hrf_thr)
    ...
    for task in TASKS:
        sel_acqs = list(np.concatenate([np.arange(i-1,j+1) for i,j in zip(task_onsets[task],task_offsets[task])]))
        roits['Edge TS ['+task+']'] = data_df[data_df[x_coord_name].isin(sel_acqs)].groupby(y_coord_name).agg(root_sum_of_squares)[data_type]
        
    # Extract min and max if not provided
    if roits_min is None:
        roits_min = roits['Edge TS ['+time_segment+']'].min()*0.95
    if roits_max is None:
        roits_max = roits['Edge TS ['+time_segment+']'].max()*1.05
    # Adding columns with edge information in such a way that the seed ROI is always the first ROI (x)
    # and the second ROI is the other ROI. This is needed to ensure consistency in coloring
    roi_idx_2_roix       = {v:k.split('-')[0] for k,v in y_name_2_y_idx.items()}
    roi_idx_2_roiy       = {v:k.split('-')[1] for k,v in y_name_2_y_idx.items()}
    roi_idx_2_roix_hemi  = {v:k.split('-')[0].split('_',2)[0] for k,v in y_name_2_y_idx.items()}
    roi_idx_2_roix_nw    = {v:k.split('-')[0].split('_',2)[1] for k,v in y_name_2_y_idx.items()}
    roi_idx_2_roix_name  = {v:k.split('-')[0].split('_',2)[2] for k,v in y_name_2_y_idx.items()}
    roi_idx_2_roiy_hemi  = {v:k.split('-')[1].split('_',2)[0] for k,v in y_name_2_y_idx.items()}
    roi_idx_2_roiy_nw    = {v:k.split('-')[1].split('_',2)[1] for k,v in y_name_2_y_idx.items()}
    roi_idx_2_roiy_name  = {v:k.split('-')[1].split('_',2)[2] for k,v in y_name_2_y_idx.items()}
    for e in roits['edge'].values:
        if roi_idx_2_roix[e] == seed_roi:
            roits.loc[e,'ROIx_Hemisphere'] = roi_idx_2_roix_hemi[e]
            roits.loc[e,'ROIx_Network']    = roi_idx_2_roix_nw[e]
            roits.loc[e,'ROIx_ROI']        = roi_idx_2_roix_name[e]
            roits.loc[e,'ROIy_Hemisphere'] = roi_idx_2_roiy_hemi[e]
            roits.loc[e,'ROIy_Network']    = roi_idx_2_roiy_nw[e]
            roits.loc[e,'ROIy_ROI']        = roi_idx_2_roiy_name[e]
        else:
            roits.loc[e,'ROIx_Hemisphere'] = roi_idx_2_roiy_hemi[e]
            roits.loc[e,'ROIx_Network']    = roi_idx_2_roiy_nw[e]
            roits.loc[e,'ROIx_ROI']        = roi_idx_2_roiy_name[e]
            roits.loc[e,'ROIy_Hemisphere'] = roi_idx_2_roix_hemi[e]
            roits.loc[e,'ROIy_Network']    = roi_idx_2_roix_nw[e]
            roits.loc[e,'ROIy_ROI']        = roi_idx_2_roix_name[e]
    roits = roits.hvplot.scatter(y=y_coord_name, x='Edge TS ['+time_segment+']', c='ROIy_Network', 
                                 cmap=nw_color_map, 
                                 hover_cols=['ROIx_Hemisphere','ROIx_Network','ROIx_ROI','ROIy_Hemisphere','ROIy_Network','ROIy_ROI'], 
                                 xlim=(roits_min,roits_max), ylim=(0,Nrois), legend=False, height=int(0.83 * height), width=int(0.10 * width)).opts(toolbar='above')
    # Seed ROI Timeseries
    # ===================
    #sbj,run,denoising,zscoring = data.name
    #aux = roi_ts_xrs[(sbj,run,denoising)].sel(dt=echo,roi=seed_roi).to_dataframe()
    #aux.drop(['roi','dt'],axis=1, inplace=True)
    ...
    # Construct Final Figure
    # ======================
    out_figure = pn.Row(pn.Column(cplot,rssts,seed_roi_ts_plot),pn.Column(roits,pn.Spacer(styles=dict(background='green'))))
    return out_figure
